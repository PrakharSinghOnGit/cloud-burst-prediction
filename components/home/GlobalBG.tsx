...
export const GlobalBG = () => {
  ...
  >([]);
  const [clouds, setClouds] = useState<Array<{ id: number; top: string; scale: number; duration: number; delay: number; opacity: number }>>([]);
  const { theme } = useTheme();
  const [heavyDrops, setHeavyDrops] = useState<
    Array<{ id: number; left: string; height: string; duration: number; delay: number; x: number }>
  >([]);
  ...
  useEffect(() => {
    ...
    const cloudLayer = Array.from({ length: isLight ? 14 : 10 }, (_, i) => ({
      ...
      opacity: (isLight ? 0.45 : 0.25) + Math.random() * (isLight ? 0.45 : 0.35),
    }));
