...
const DynamicMap = dynamic(async () => {
  ...
  const { MapContainer, TileLayer, Marker, Popup, Circle, Rectangle } = await import("react-leaflet");

  return function InnerMap({ userLocation }: { userLocation: [number, number] | null }) {
    ...
    return (
      <MapContainer center={center} zoom={userLocation ? 15 : 12} scrollWheelZoom={false} style={{ height: "100%", width: "100%", borderRadius: 12 }}>
        <TileLayer
        ...
        <Circle center={[28.61, 77.205]} radius={1200} pathOptions={{ color: "#ef4444", fillOpacity: 0.15 }} />
      </MapContainer>
    );
  };
}, { ssr: false });
  ...
  return (
    <Card className="w-full h-96">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button size="icon" variant="outline" aria-label="Open map fullscreen" onClick={() => setOpen(true)}>
          <Maximize2 className="h-4 w-4" />
        </Button>
      </CardHeader>
      <CardContent className="h-80">
        {mounted ? <DynamicMap userLocation={userLocation} /> : <div className="h-full w-full rounded-xl bg-muted" />}
      </CardContent>
